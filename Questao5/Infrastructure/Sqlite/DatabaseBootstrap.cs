...
namespace Questao5.Infrastructure.Sqlite
{
    public class DatabaseBootstrap : IDatabaseBootstrap
    {
        ...
        public void Setup()
        {
            ...
            var existingTables = connection.Query<string>("SELECT name FROM sqlite_master WHERE type='table';").ToList();

            if (!existingTables.Contains("contacorrente"))
            {
            ...
            if (!existingTables.Contains("movimento"))
            {
            ...
            if (!existingTables.Contains("idempotencia"))
            {
            ...
            var contacorrenteExists = connection.QueryFirstOrDefault<int>("SELECT COUNT(*) FROM contacorrente") > 0;
            if (!contacorrenteExists)
            {
